...
vi.mock('../../src/services/forecast', () => ({
  fetchPointInspection: vi.fn().mockResolvedValue({
    status: 'ready',
    summary: 'Temperature near this point is 18°C.',
...
describe('Overlay failure states', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('keeps the base map usable when overlay resolution fails', async () => {
    resolveOverlayTileUrl.mockRejectedValueOnce(new Error('Overlay data is unavailable.'))
    ...
    expect(await screen.findByLabelText(/mock weather map canvas/i, {}, { timeout: 5000 })).toBeInTheDocument()
    expect(await screen.findByRole('alert', {}, { timeout: 5000 })).toHaveTextContent(/overlay data is unavailable/i)
  }, 10000)

  it('shows the fallback summary when the map runtime fails', async () => {
    resolveOverlayTileUrl.mockResolvedValue('https://example.com/overlay.png')

    render(<App />)
    await userEvent.type(screen.getByLabelText(/location search/i), 'Los Angeles')
    await userEvent.click(screen.getByRole('button', { name: /^search$/i }))
    await userEvent.click(await screen.findByRole('tab', { name: 'Map' }))
    await userEvent.click(await screen.findByRole('button', { name: /trigger map error/i }, { timeout: 5000 }))

    expect(await screen.findByLabelText(/map fallback summary/i, {}, { timeout: 5000 })).toHaveTextContent(/map unavailable/i)
  })
})